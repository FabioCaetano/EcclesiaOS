...
116. Permissao de evento por grupo/ministerio responsavel: helpers `isUserResponsibleForEvent`, `canEditEvent`, `canCreateEventDraft` e `leadsAnyGroup` em `@ecclesiaos/shared`; backend permite admin ou lider de `event.groupId`/algum `event.requestedTeamIds` criar e atualizar eventos; quando o ator nao e admin, `groupId`, `requestedTeamIds` e `operatorPersonIds` sao preservados pelo servidor mesmo se o input tentar alterar (bloqueia escalada); excluir continua admin-only; frontend usa o mesmo helper para mostrar/desabilitar form e botao "Novo evento"; sem migration, sem novo campo.
117. Centro de notificacoes interno: tipos `NotificationKind` e `NotificationItem` em `@ecclesiaos/shared`; backend agrega quatro fontes derivadas (`serving_pending`, `serving_declined`, `event_upcoming`, `registration_email_pending`) a partir dos dados existentes sem nova tabela; endpoint `GET /notifications` retorna a lista ordenada; frontend exibe sino com badge no header (`AppLayout`) e painel dropdown com items agrupados por tipo; "lido" guardado em `localStorage` por usuario (chave `ecclesiaos:notifications:lastReadAt:<userId>`) — sem persistencia no banco; refresh automatico a cada 60s; clique em item navega para a view do modulo de origem; tipos `kind` desconhecidos sao ignorados no frontend (futuro-proof).
118. Upload de logo da igreja: campo `ChurchProfile.logoDataUrl: string` guarda data URL base64 (PNG/JPEG/SVG/WebP, limite 100 KB) em vez de usar storage externo (S3) ou filesystem (efemero em Render/Vercel); util `validateLogoDataUrl` compartilhado entre frontend e backend; `sanitizeChurchUpdate` valida prefixo e tamanho rejeitando com 400 caso invalido; novo endpoint **publico** `GET /public/church-info` retorna apenas `{ name, logoDataUrl }`; `AppLayout` e `LoginPage` usam logo dinamico com fallback para o asset `/ecclesia-os-logo-cropped.png`; migration Prisma `20260511120000_church_logo` adiciona coluna `logoDataUrl String @default("")` em `ChurchProfileRecord`.
119. Popup de acoes no calendario: clicar numa pill do calendario (no grid ou na lista do dia) abre um popover contextual com acoes; evento tem `Abrir Culto` (todos com acesso a `serviceOps`), `Editar agenda` (`canEditEvent` da Fase 116) e `Excluir agenda` (admin); reserva tem `Abrir Ambientes`; ESC e click fora fecham; deteccao de click fora via `closest(".calendar-popover")`/`closest(".calendar-item-wrapper")`; botao "+ Novo evento/agenda" liberado para admin OU `leadsAnyGroup`; reaproveita helpers ja existentes, sem mudancas em backend, banco ou tipos.
120. Grupos com Posicoes e visibilidade: `GroupsPage` substitui textarea de `servicePositions` por chips editaveis (input por posicao + X + botao `+ Posicao`); remover posicao limpa-a tambem de `memberServicePositions`; membros sao adicionados por busca `<input list>` + `<datalist>` e exibidos como chips com toggles de posicao (multi-select); label "Posicoes de servico" vira "Posicoes" quando o tipo e `ministry` ou `team`; visibilidade da pagina filtra `groups` no frontend quando o usuario nao e admin (so ve onde `leaderPersonId === user.personId` ou `memberPersonIds.includes(user.personId)`); backend nao mexe — outros modulos (calendario, notificacoes, mensagens, escala) seguem usando a lista completa de grupos; sem migration, sem mudanca de contrato.
121. Aba Etiquetas dedicada: `AppModuleKey` ganha `"labels"` em `@ecclesiaos/shared` e entra em `adminOnlyModules` (admin-only no `canAccessModule` e `canManageModule`); novo componente `LabelsPage` recebe todo o CRUD de templates Brother (formulario, imprimir teste, render `@page` dinamico) que vivia em `ChurchPage`; nova entrada no menu lateral grupo Sistema com icone `Tag`; `ChurchPage` perdeu `templates`, `templateForm`, `editingTemplateId`, `printingTemplate`, `templateStatus`, `refreshTemplates`, `startNewTemplate`, `editTemplate`, `handleTemplateSubmit`, `handleDeleteTemplate`, `printTestLabel` e os imports correspondentes; endpoints `/label-templates` no backend permanecem inalterados; sem migration.
122. Familiares pesquisaveis em Pessoas: `PeoplePage` substitui fieldset `member-picker` (checkbox de todas as pessoas) por chips removiveis + busca `<input list="people-familiares-options">` + `<datalist>`; nomenclatura passa de "Responsaveis vinculados" para "Familiares" no frontend; campo do contrato compartilhado `guardianPersonIds: string[]` permanece intocado; pessoa em edicao e filtrada das sugestoes para evitar auto-vinculacao; busca vazia mostra mensagem "Pessoa nao encontrada"; visibilidade da aba ja era restrita a admin/lider via `canAccessModule("people")`, sem mudanca; sem migration, sem novo backend, reaproveita classes CSS `.forms-chip`, `.forms-chip-remove`, `.forms-people-chips`, `.forms-people-input` da Fase 115.
